...
bookingForm?.addEventListener('submit', async (event) => {
  ...
  if (!turnstileToken) {
    showFormStatus('error', 'Please complete the security verification and try again.');
    return;
  }
  ...
  try {
    ...
    if (!response.ok || !result.success) {
      console.error('Web3Forms rejected the request:', result);
      showFormStatus('error', result.message || "We couldn't send the request. Please try again.");
    ...
    console.error('Web3Forms AJAX error:', error);

    if (responseReceived) {
      showFormStatus('error', "We couldn't send the request. Please try again.");
      if (window.turnstile) window.turnstile.reset();
      return;
    }
